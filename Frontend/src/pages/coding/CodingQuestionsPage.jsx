...
export function CodingQuestionsPage() {
  ...
  const selectedSubtopicObj = selectedSubtopicId !== 'ALL'
    ? (subtopicsForStage.find((sub) =>
        ...
        SUBTOPIC_MODULE_MAP[cleanId(selectedSubtopicId)] === cleanId(sub.id) ||
        cleanStr(sub.title) === cleanStr(selectedSubtopicId)
      ) || null)
    : null;

  const modulesForSubtopic = selectedSubtopicObj ? getInnerModulesForSubtopic(selectedSubtopicObj, courseLessons, selectedStageObj?.id || selectedStageId) : [];
  ...
  const currentSubtopicObj = currentSubtopicsArr.find((st) =>
    st.id === formData.subtopicId ||
    cleanId(st.id) === cleanId(formData.subtopicId) ||
    SUBTOPIC_MODULE_MAP[cleanId(st.id)] === cleanId(formData.subtopicId) ||
    SUBTOPIC_MODULE_MAP[cleanId(formData.subtopicId)] === cleanId(st.id) ||
    cleanStr(st.title) === cleanStr(formData.subtopicId)
  ...
  // Helper to count questions in a stage
  const getQuestionCountForStage = React.useCallback((stageId) => {
    if (!stageId || stageId === 'ALL') return codingQuestions.length;
    return codingQuestions.filter((q) => {
      const qCourseId = q.courseId || q.course_id;
      if (activeCourseId && qCourseId && qCourseId !== 'ALL' && qCourseId !== activeCourseId) return false;
      const h = resolveHierarchy(q);
      const qStageId = h.stageId || q.stageId || q.stage_id;
      return isMatchingStage(qStageId, stageId) || (h.stageNum !== null && getStageNumber(stageId) === h.stageNum);
    }).length;
  }, [codingQuestions, activeCourseId, resolveHierarchy]);

  // Helper to count questions in a module
  const getQuestionCountForModule = React.useCallback((subId, subTitle) => {
    return codingQuestions.filter((q) => {
      const qCourseId = q.courseId || q.course_id;
      if (activeCourseId && qCourseId && qCourseId !== 'ALL' && qCourseId !== activeCourseId) return false;
      const h = resolveHierarchy(q);
      const qSubId = h.subtopicId || q.subtopicId || q.subtopic_id;
      return (
        cleanId(qSubId) === cleanId(subId) ||
        SUBTOPIC_MODULE_MAP[cleanId(qSubId)] === cleanId(subId) ||
        SUBTOPIC_MODULE_MAP[cleanId(subId)] === cleanId(qSubId) ||
        cleanStr(q.subtopicName) === cleanStr(subTitle) ||
        cleanStr(qSubId) === cleanStr(subTitle) ||
        cleanId(qSubId) === cleanId(subId)
      );
    }).length;
  }, [codingQuestions, activeCourseId, resolveHierarchy]);

  // Helper to count questions in a submodule
  const getQuestionCountForSubmodule = React.useCallback((modId, modTitle) => {
    return codingQuestions.filter((q) => {
      const qCourseId = q.courseId || q.course_id;
      if (activeCourseId && qCourseId && qCourseId !== 'ALL' && qCourseId !== activeCourseId) return false;
      const h = resolveHierarchy(q);
      const qModId = h.moduleId || q.innerTopicId || q.inner_topic_id || q.moduleId || q.module_id;
      return (
        cleanId(qModId) === cleanId(modId) ||
        cleanStr(q.moduleName) === cleanStr(modTitle) ||
        cleanStr(q.topicName) === cleanStr(modTitle) ||
        cleanStr(qModId) === cleanStr(modTitle)
      );
    }).length;
  }, [codingQuestions, activeCourseId, resolveHierarchy]);
  ...
  const handleOpenAddModal = (presetSubtopicId = null, presetModuleId = null) => {
    ...
    const activeStage = (selectedStageId !== 'ALL' && nextStages.find(s => s.id === selectedStageId || isMatchingStage(s.id, selectedStageId))) || nextStages[0];
    const activeSubs = getSubtopicsForStage(activeStage);
    const targetSubId = presetSubtopicId || selectedSubtopicId;
    const activeSub = (targetSubId !== 'ALL' && activeSubs.find(s => s.id === targetSubId || cleanId(s.id) === cleanId(targetSubId) || SUBTOPIC_MODULE_MAP[cleanId(s.id)] === cleanId(targetSubId) || SUBTOPIC_MODULE_MAP[cleanId(targetSubId)] === cleanId(s.id) || cleanStr(s.title) === cleanStr(targetSubId))) || activeSubs[0];
    const activeInners = getInnerModulesForSubtopic(activeSub, courseLessons, activeStage?.id);
    const targetInnerId = presetModuleId || selectedModuleId;
    const activeInner = (targetInnerId !== 'ALL' && activeInners.find(m => m.id === targetInnerId || cleanId(m.id) === cleanId(targetInnerId) || cleanStr(m.title) === cleanStr(targetInnerId))) || null;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* CURRICULUM HIERARCHY EXPLORER: STAGE -> MODULES -> SUBMODULES */}
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-2xs overflow-hidden">
        {selectedStageId === 'ALL' ? (
          // 4-Stage Quick Select Overview when "All Stages" is selected
          <div className="p-5 space-y-3 bg-gradient-to-r from-slate-50 via-blue-50/20 to-purple-50/20">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <h3 className="text-xs font-black uppercase tracking-wider text-slate-700 flex items-center gap-2">
                  <Layers className="w-4 h-4 text-blue-600" />
                  Curriculum Stages Explorer
                </h3>
                <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                  Select a specific stage to view all its modules and submodules
                </p>
              </div>
              <span className="text-[11px] font-bold text-slate-400 bg-white px-2.5 py-1 rounded-xl border border-slate-200/60 shadow-2xs self-start sm:self-auto">
                {activeStagesList.length} Stages Available
              </span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-1">
              {activeStagesList.map((stg, idx) => {
                const stgSubs = getSubtopicsForStage(stg);
                const qCount = getQuestionCountForStage(stg.id);
                return (
                  <button
                    key={stg.id}
                    type="button"
                    ...
                    className="p-4 rounded-2xl bg-white border border-slate-200/90 hover:border-blue-500 hover:shadow-md transition-all text-left group cursor-pointer space-y-2.5"
                  >
                    <div className="flex items-center justify-between">
                      <span className="px-2.5 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider bg-blue-50 text-blue-700 border border-blue-100 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                        Stage {idx + 1}
                      </span>
                      <span className="text-[11px] font-extrabold text-slate-500 bg-slate-100 group-hover:bg-blue-50 group-hover:text-blue-700 px-2 py-0.5 rounded-md transition-colors">
                        {qCount} Qs
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 text-[11px] text-slate-500 font-medium pt-0.5">
                      ...
                      <span>{stgSubs.length} Modules</span>
                    </div>
                  </button>
                );
              })}
          ...
          // Active Stage selected: Display All Modules and Submodules
          <div className="p-5 space-y-4">
            {/* Stage Header Banner */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-2xl bg-blue-50 border border-blue-200 flex items-center justify-center text-blue-600 flex-shrink-0 shadow-2xs">
                  <Layers className="w-4 h-4" />
                </div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-sm sm:text-base font-extrabold text-slate-900">
                      {selectedStageObj?.title || 'Selected Stage'}
                    </h3>
                    <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-blue-100 text-blue-700 border border-blue-200/60">
                      {subtopicsForStage.length} Modules in Stage
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                    Select any module below to display its specific submodules and filter coding problems
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl border border-slate-200 text-xs font-bold text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-all self-start sm:self-auto cursor-pointer flex items-center gap-1.5 shadow-2xs"
                >
            ...
            {/* 1. MODULES PRESENT IN THIS SPECIFIC STAGE */}
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                  <Bookmark className="w-3.5 h-3.5 text-emerald-600" />
                  Modules in this Stage ({subtopicsForStage.length})
                </span>
                {selectedSubtopicId !== 'ALL' && (
                  <button
                    type="button"
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                ...
                >
                  ...
                  <span className={`px-1.5 py-0.5 rounded-md text-[10px] font-black ${
                    selectedSubtopicId === 'ALL' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-600'
                  }`}>
                    {getQuestionCountForStage(selectedStageId)}
                  </span>
                </button>

                {subtopicsForStage.map((sub, idx) => {
                  const isSelected = selectedSubtopicId === sub.id || cleanId(selectedSubtopicId) === cleanId(sub.id);
                  const qCount = getQuestionCountForModule(sub.id, sub.title);
                  return (
                    <button
                      key={sub.id || idx}
                      type="button"
                      ...
                      className={`px-3.5 py-2 rounded-2xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer border shadow-2xs ${
                        isSelected
                          ...
                          : 'bg-white hover:bg-emerald-50/70 text-slate-800 border-slate-200 hover:border-emerald-300'
                      }`}
                    >
                      ...
                      <span className="truncate max-w-[220px]">{sub.title}</span>
                      <span className={`px-1.5 py-0.5 rounded-md text-[10px] font-black ${
                        isSelected ? 'bg-white/20 text-white' : 'bg-emerald-50 text-emerald-700 border border-emerald-200/60'
                      }`}>
                        {qCount}
                      </span>
                    </button>
            ...
            {/* 2. SPECIFIC SUBMODULES IN SELECTED MODULE (displayed when module is selected) */}
            {selectedSubtopicId !== 'ALL' && selectedSubtopicObj && (
              <div className="p-4 bg-purple-50/50 rounded-2xl border border-purple-200/80 space-y-3 animate-in fade-in slide-in-from-top-1 duration-200 shadow-2xs">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-lg bg-purple-600 text-white flex items-center justify-center text-xs font-black flex-shrink-0">
                      <Sparkles className="w-3.5 h-3.5" />
                    </div>
                    <div>
                      <span className="text-xs font-black text-purple-900 uppercase tracking-wider">
                        Submodules in &ldquo;{selectedSubtopicObj.title}&rdquo;
                      </span>
                      <span className="ml-2 text-[11px] text-purple-700 font-bold bg-purple-100 px-2 py-0.5 rounded-md border border-purple-200">
                        {modulesForSubtopic.length} Submodules
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {selectedModuleId !== 'ALL' && (
                      <button
                        ...
                        className="text-[11px] font-bold text-purple-700 hover:text-purple-900 cursor-pointer underline"
                      >
                        All Submodules
                      </button>
                    )}
                    <button
                      type="button"
                      onClick={() => handleOpenAddModal(selectedSubtopicObj.id, selectedModuleId !== 'ALL' ? selectedModuleId : null)}
                      className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-100/90 hover:bg-emerald-200 border border-emerald-300 px-2.5 py-1 rounded-xl transition-colors cursor-pointer shadow-2xs"
                    >
                      <Plus className="w-3 h-3 text-emerald-600" />
                      <span>Post in Module</span>
                    </button>
                  </div>
                </div>

                {modulesForSubtopic.length > 0 ? (
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer border ${
                        selectedModuleId === 'ALL'
                          ...
                          : 'bg-white hover:bg-purple-100/70 text-slate-700 border-purple-200'
                      }`}
                    >
                      <span>All Submodules</span>
                      <span className={`px-1.5 py-0.2 rounded-md text-[10px] font-black ${
                        selectedModuleId === 'ALL' ? 'bg-white/20 text-white' : 'bg-purple-100 text-purple-700'
                      }`}>
                        {getQuestionCountForModule(selectedSubtopicObj.id, selectedSubtopicObj.title)}
                      </span>
                    </button>

                    {modulesForSubtopic.map((mod, mIdx) => {
                      const isModSelected = selectedModuleId === mod.id || cleanId(selectedModuleId) === cleanId(mod.id);
                      const qCount = getQuestionCountForSubmodule(mod.id, mod.title);
                      return (
                        <button
                          key={mod.id || mIdx}
                          type="button"
                          ...
                          className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer border ${
                            isModSelected
                              ? 'bg-purple-600 text-white border-purple-600 shadow-xs'
                              : 'bg-white hover:bg-purple-100/70 text-slate-800 border-purple-200 hover:border-purple-300'
                          }`}
                        >
                          <span className={`text-[10px] font-black px-1.5 py-0.5 rounded ${
                            isModSelected ? 'bg-white/20 text-white' : 'bg-purple-100 text-purple-700'
                          }`}>
                            {mIdx + 1}
                          </span>
                          <span className="truncate max-w-[220px]">{mod.title}</span>
                          <span className={`px-1.5 py-0.2 rounded-md text-[10px] font-black ${
                            isModSelected ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'
                          }`}>
                            {qCount}
                          </span>
                        </button>
                  ...
                  <p className="text-xs text-slate-500 italic">
                    No granular submodules configured for this module.
                  </p>
                )}
              </div>
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="bg-white/95 p-2.5 sm:p-3 rounded-xl border border-emerald-100/90 shadow-2xs">
                    <CustomSelect
                      ...
                      onChange={(e) => {
                        ...
                        const targetSub = currentSubtopicsArr.find((st) =>
                          st.id === newSubId ||
                          cleanId(st.id) === cleanId(newSubId) ||
                          SUBTOPIC_MODULE_MAP[cleanId(st.id)] === cleanId(newSubId) ||
                          SUBTOPIC_MODULE_MAP[cleanId(newSubId)] === cleanId(st.id) ||
                          cleanStr(st.title) === cleanStr(newSubId)
                        ) || currentSubtopicsArr[0];
                        setFormData((prev) => ({
                          ...prev,
                          subtopicId: newSubId,
                  ...
                  <div className="bg-white/95 p-2.5 sm:p-3 rounded-xl border border-emerald-100/90 shadow-2xs">
                    <CustomSelect
                      ...
                      options={[
                        { value: '', label: 'None (Module Level / All Submodules)' },
